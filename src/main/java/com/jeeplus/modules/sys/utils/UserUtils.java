...
public class UserUtils {
	...
	public static final String USER_OFFICE_CACHE = "userOfficeCache";
	public static final String USER_OFFICE_CACHE_ID_ = "user_and_office_id_";
	...
	public static final String CACHE_USER_MENU__LIST = "menuUseList";
	...
	public static void clearCache(User user){
		...
		CacheUtils.remove(USER_OFFICE_CACHE, USER_CACHE_ID_ + user.getId());
	...
	public static Office getUserOffice(){
		User user = getUser();
		Office userOffice = (Office) CacheUtils.get(USER_OFFICE_CACHE, USER_OFFICE_CACHE_ID_ + user.getId() + user.getOffice().getId());
		if (null == userOffice) {
			userOffice = officeDao.get(user.getOffice().getId());
			CacheUtils.put(USER_OFFICE_CACHE, USER_CACHE_ID_ + user.getId() + user.getOffice().getId(), userOffice);
		}
		return userOffice;
	}
	...
	public static String getUserGrade(){
		return getUserOffice().getGrade();
	}
	
	/**
	 * 获取当前用户的父层级 Grade
	 */
	public static String getUserParentGrade(){
		return getUserOffice().getParentId();
	}
	
	/**
	 * 获取当前用户的上层部门的名称
	 */
	public static String getUserParentDeptName(){
		String pDeptId = getUserOffice().getParentId();
		Office pOffice = (Office)officeDao.get(pDeptId);
		return pOffice.getName();
	}
	 ...
	 * @return
	 */
	public static List<Office> getSameGradeOffice(){
		@SuppressWarnings("unchecked")
		...
		List<Office> offices = officeDao.findListByParam(office);
...
//		getCacheMap().remove(key);
	 ...
	 * @return
	 */
	public static Office getUserOffice(User user){
		user = userDao.get(user.getId());
		return officeDao.get(user.getOffice().getId());
	}

	/**
	 * 根据传入的 userId 获取他的所属机构
	 * @param userId
	 * @return
	 */
	public static String getOfficeIdByUserId(String userId){
		User user = userDao.get(userId);
		if (null == user || null == user.getOffice()) {
			return "";
		}
		return user.getOffice().getId();
	}
	...
	public static List<Menu> getMenuListByParentId(String parentId) {
		@SuppressWarnings("unchecked")
		List<Menu> menuList = (List<Menu>) getCache(CACHE_USER_MENU__LIST + parentId);
		if (menuList == null) {
			User user = getUser();
			if (user.isAdmin()) {
				menuList = menuDao.findByAdmin("1");
			} else {
				Menu m = new Menu();
				m.setUserId(user.getId());
				menuList = menuDao.findByUserIdAndParentId(user.getId(), parentId);
			}

			putCache(CACHE_USER_MENU__LIST + parentId, menuList);
		}
		return menuList;
	}
}
