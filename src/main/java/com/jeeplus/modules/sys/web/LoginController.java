...
//		}
		...
		// 如果是手机没有登录跳转到到login，则返回JSON字符串
		 ...
		 //return "redirect:http://localhost:8090/login/reLoad.do";
//		return "redirect:http://lsxt.hazw.gov.cn/login/reLoad.do";
	}
...
//			return "redirect:http://lsxt.hazw.gov.cn/login/reLoad.do";
	...
	public String index(HttpServletRequest request, HttpServletResponse response) {
		...
		Collection<Session> sessions = sessionDAO.getActiveSessions();
		 for(Session session:sessions){
          session.setTimeout(1800000);  //这里就把session有效期设置为半个小时
       }
		if (logger.isDebugEnabled()){
...
//		System.out.println("==========================b");
		//
		List<Menu> menuList = systemService.getMainMenu();
	 ...
	 * 查看未报送和未调度
	 */
	@RequestMapping(value = "${adminPath}/login/getCountTsxx")
