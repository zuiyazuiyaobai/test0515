...
//显示提示框
function showTipCur(mess, type, timeout, lazytime){
	resetTipCur();
	setTimeout(function(){
		$.jBox.tip(mess, (type == undefined || type == '' ? 'info' : type), {opacity:0,
			timeout:  timeout == undefined ? 2000 : timeout});
	}, lazytime == undefined ? 100 : lazytime);
}

//显示提示框
function showTipTop(mess, type, timeout){
	resetTipCur();
	top.$.jBox.tip(mess, (type == undefined || type == '' ? 'info' : type), {opacity:0,
...
//打开对话框(添加修改)
function openDialogWithCallback(title,url,width,height,target){
	if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {//如果是移动端，就使用自适应大小弹窗
		width = 'auto';
		height = 'auto';
	}
...
//打开对话框(查看)
function openDialogViewWithCallBack(title,url,width,height){
    if(navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){//如果是移动端，就使用自适应大小弹窗
        width='auto';
        height='auto';
    }else{//如果是PC端，根据用户设置的width和height显示。
...
//根据 ID 返回指定 form 中的所有的 <input><textarea><select> 的 name-value 对象
...
function addSelectElement(data,element){
	var value = $(element).find("option:selected").val();
	if(null != element.name && null != value){
		data[element.name] = value;
	}
}

function addElement(data,element){
	if(null != element.value && "" != element.value){
		data[element.name] = element.value;
	}
}

function addNumberElement(data,element){
...
/**
 * 比较 small 与 big，如果 small 大于 big ，则调换双方的值
 * smallId: 需要结果为小的 Id
 * bigId: 需要结果为大的 Id
 */
function compareExchange(smallId,bigId){
    var small = Number($("#" + smallId).val());
    var big = Number($("#" + bigId).val());
    if(null != small && null != big && small > big){
		$("#" + smallId).val(big);
		$("#" + bigId).val(small);
    }
}