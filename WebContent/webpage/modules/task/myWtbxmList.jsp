...
<head>
    ...
    <script type="text/javascript">
        ...
        function stwtb(){
        	var wtb = 1;
        	var url = ctx + "/task/cInsBusinessXmjbxxTaskRelation/myWtbxmList?para="+wtb;
        ...
        function toyj(){
        	var ids="";
        	var str="";
        	$("input[name='toyj']:checkbox:checked").each(function () {
            ...
            $.post(ctx + "/xmjbxx/xmjbxx/xmyjFsxxOne", {ids:ids}, function (data) {
                if (data==0) {
                   top.layer.alert('预警提示成功!', {icon: 0, title: '提示'});
                   sortOrRefresh();
                }else {
                   top.layer.alert('预警提示失败!', {icon: 0, title: '提示'});
                }
            });
    ...
    <div class="ibox ibox-custom">
        ...
        <div class="ibox-content">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <form:form id="searchForm" modelAttribute="xmjbxxTaskRelationSearchParameter"
                               action="${ctx}/task/cInsBusinessXmjbxxTaskRelation/myWtbxmList" method="post" class="form-inline">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="pull-left">
                       <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="stwtb()" title="三天未填报"><i
						        class="fa fa-pencil-square-o"></i> ${label==null?'三天未填报':label}</button>
						</button>
						<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="toyj()" title="预警"><i
						        class="fa fa-pencil-square-o"></i> ${label==null?'预警':label}</button>
						</button>
						<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="sortOrRefresh()" title="刷新"><i
						        class="fa fa-pencil-square-o"></i> ${label==null?'刷新':label}</button>
						</button>
                    </div>
            ...
            <table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
                ...
                <c:forEach items="${page.list}" var="relation">
                    ...
                    <tr>
                        <td><input type="checkbox" name="toyj"  id="${relation.xmjbxx.id}" class="i-checks toyj"></td>
                        <td>
                                ${status.index + 1}
                        </td>
                        <td><a href="#" onclick="openDialogViewWithCallBack('查看调度反馈', '${ctx}/task/cInsBusinessXmjbxxTaskRelation/taskFeedback?id=${relation.id}&xmjbxxid=${xmjbxx.id}','800px', '600px')">
