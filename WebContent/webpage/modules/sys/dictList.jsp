		...
		<div class="form-group">
			<span>类型：</span>
				<%--<form:select id="type" path="type" class="form-control m-b">
					<form:option value="" label=""/><form:options items="${dict}" htmlEscape="false"/>
				</form:select>--%>
			<select id="type" name="type" class="form-control m-b">
				<option value=""></option>
				<c:forEach var="dictItem" items="${dictList}">
					<option value="${dictItem.type}" <c:if test="${dict.type == dictItem.type}">selected</c:if>>${dictItem.type}[${dictItem.description}]</option>
				</c:forEach>
			</select>
			<span>描述 ：</span>
	...
	<div class="col-sm-12">
		<div class="pull-left">
			...
			<shiro:hasPermission name="sys:dict:edit">
			    <table:editRow url="${ctx}/sys/dict/edit" id="contentTable"  title="字典"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
		...
		<c:forEach items="${page.list}" var="dict">
			<tr>
				...
				<td>
					...
					<shiro:hasPermission name="sys:dict:edit">
    					<a href="#" onclick="openDialog('修改字典', '${ctx}/sys/dict/edit?id=${dict.id}','800px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="sys:dict:del">
					...
					<shiro:hasPermission name="sys:dict:add">
						<a href="#" onclick="openDialog('添加键值', '<c:url value='${fns:getAdminPath()}/sys/dict/form?id=${dict.id}&type=${dict.type}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>','800px', '500px')" class="btn btn-primary btn-xs" ><i class="fa fa-plus"></i> 添加键值</a>
					</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
