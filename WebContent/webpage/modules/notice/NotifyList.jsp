...
<div class="ibox-title">
	...
	<div class="row">
	<div class="col-sm-12">
		<form:form id="searchForm" modelAttribute="notify" action="${ctx}/notice/notify/${notify.self?'self':''}?noticeType=1" method="post" class="form-inline">
	...
	<div class="col-sm-12">
		<div class="pull-left">
			...
			<shiro:hasPermission name="notice:notify:add">
				<table:addRow url="${ctx}/notice/notify/form?noticeType=1" title="通知" width="1000px" height="600px"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission>
			<%-- <shiro:hasPermission name="notice:notify:edit">
			    <table:editRow url="${ctx}/notice/notify/form?noticeType=1" id="contentTable"  title="通知" width="800px" height="700px"></table:editRow><!-- 编辑按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="notice:notify:del">
				<table:delRow url="${ctx}/notice/notify/deleteAll?noticeType=1" id="contentTable"></table:delRow><!-- 删除按钮 -->
			</shiro:hasPermission> --%>
	...
	<table id="contentTable" class="table table-striped table-bordered  table-hover table-condensed  dataTables-example dataTable no-footer">
		...
		<c:forEach items="${page.list}" var="notify">
			<tr>
				...
				<td>
					${fns:getDictLabel(notify.type, 'oa_notify_type', '')}
				</td>
				<td>
				...
				<c:if test="${!requestScope.notify.self}">
					<shiro:hasPermission name="notice:notify:view">
						<a href="#" onclick="openDialogView('查看通知', '${ctx}/notice/notify/form?id=${notify.id}&noticeType=1','1000px', '600px')" class="btn btn-info btn-xs btn-circle" ><i class="fa fa-search-plus"></i></a>
					</shiro:hasPermission>
					<shiro:hasPermission name="notice:notify:edit">
    					<a href="#" onclick="openDialog('修改通知', '${ctx}/notice/notify/form?id=${notify.id}&noticeType=1','1000px', '600px')" class="btn btn-success btn-xs btn-circle" ><i class="fa fa-edit"></i></a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="notice:notify:del">
