		...
		function shqr() {
		    ...
		    $("#contentTable tbody tr td input.i-checks:checkbox").each(function () {
		        if (true == $(this).is(':checked')) {
		            str += $(this).attr("id") + ",";
		            var remarke=$(this).parent().next().val();
		            if(remarke=="2017年省基建项目"||remarke=="2018年省基建项目"){
		            	sfsjjFlag="1";
		            }
			    }
		    });
		    ...
		    if(idArr.length==2&&sfsjjFlag=="1"){
		    	top.layer.alert('省基建项目只能选中一个进行报送!', {icon: 0, title: '警告'});
		    	return;
		    }
			top.layer.open({
				type: 2,
				...
				title: '调度任务',
				maxmin: true, //开启最大化最小化按钮
				content: '${ctx}/task/schedulerTask/form?origin=' + ids+'&sfsjjFlag='+sfsjjFlag,
				btn: ['确定', '关闭'],
				yes: function(index, layero){
					var body = top.layer.getChildFrame('body', index);
					var iframeWin = layero.find('iframe')[0]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="xmjbxxSearchParameter" action="${ctx}/xmjbxx/xmjbxx/tzjhZbzkList" method="post" class="form-inline">
		...
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			...
			<input id="ztzMin" name="ztzMin" onchange="compareExchange('ztzMin','ztzMax')" type="number" class="number form-control" value="${xmjbxxSearchParameter.ztzMin}" style="width:120px;"/> -
			<input id="ztzMax" name="ztzMax" onchange="compareExchange('ztzMin','ztzMax')" type="number" class="form-control number" value="${xmjbxxSearchParameter.ztzMax}" style="width:120px;"/>
		</div>
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>符合重大战略：</span>
		...
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			...
			<input type="hidden" name="updateDateStart" id="updateDateStart" value="<fmt:formatDate value='${xmjbxxSearchParameter.updateDateStart}' pattern='yyyy-MM-dd'/>">
			<input type="hidden" name="updateDateEnd" id="updateDateEnd" value="<fmt:formatDate value='${xmjbxxSearchParameter.updateDateEnd}' pattern='yyyy-MM-dd'/>">
			<input id="updateDate" name="updateDate" type="text" class="laydate-icon-img form-control layer-date input-sm" value="${xmjbxxSearchParameter.updateDateBT}" style="width:220px;"/>
		</div>
		...
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>审核辅助标志01：</span>
			<form:input path="shfzbsone" htmlEscape="false" maxlength="200" class=" form-control input-sm"/>
		</div>
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>审核辅助标志02：</span>
			<form:input path="shfzbstwo" htmlEscape="false" maxlength="200" class=" form-control input-sm"/>
		</div>
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>审核辅助标志03：</span>
			<form:input path="shfzbsthree" htmlEscape="false" maxlength="200" class=" form-control input-sm"/>
		 </div>
		 <div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>审核部门：</span>
			<form:input path="shbm.name"  htmlEscape="false" maxlength="200" class=" form-control input-sm"/>
		 </div>
		 </div>
	</form:form>
	<br/>
	...
	<div class="col-sm-12">
		<div class="pull-left" style="display:none">
			...
			<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="txxm()" title="从储备库挑选项目"><i class="fa fa-plus"></i>从储备库挑选项目</button>
			</shiro:hasPermission>
			...
			<shiro:hasPermission name="xmjbxx:xmjbxx:tzjhDbqList_sb">
				<table:operatePanel url="${ctx}/xmjbxx/xmjbxx/tzjhDbqSb" id="contentTable" nextOffices="${nextOffices}" nextOfficesName="${nextOfficesName}" checkFunc="checkSb" panelName="bswh" operateIndex="1" label="上报"></table:operatePanel>
			</shiro:hasPermission>
			<table:operateRow url="${ctx}/xmjbxx/xmjbxx/sngdjhThdbq2" id="contentTable" operateIndex="2" label="退回储备库"></table:operateRow>
			...
			<shiro:hasPermission name="xmjbxx:xmjbxx:cbxmShqList_shtg">
				<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="shqr()" title="审核确认"><i class="fa fa-pencil-square-o"></i> 审核确认</button>
			</shiro:hasPermission>
    ...
    <table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
        ...
        <c:forEach items="${page.list}" var="xmjbxx">
            <tr>
				<td><input type="checkbox" id="${xmjbxx.id}" sztAttr="${xmjbxx.szt}" class="i-checks">
					<input type="hidden" value="${xmjbxx.remarks}"/>
				</td>
                ...
                <td>
                ${fns:getDictLabel(xmjbxx.sfsqzyysnzj, 'yes_no', '')}
			    </td>
