...
<script type="text/javascript">
	...
	function checkQxcb(ids) {
		...
		for ( var i = 0; i < idsArr.length; i++) {
			...
			if ('2' === nrzt) {
				top.layer.alert("项目（" + xmmc + "）已经被挑入滚动计划编制区的项目，不能取消储备！", {
					icon : 0
				});
				return false;
			}
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm"
							modelAttribute="deptRelationSearchParameter"
							action="${ctx}/xmjbxx/xmjbxx/cbxmCbkList" method="post"
							class="form-inline">
							...
							<input id="moreSearch" name="moreSearch" type="hidden"
								value="${deptRelationSearchParameter.moreSearch}" />
							<!-- 是否展开更多查询 -->
							<div class="form-group">
								<div class="form-group right10">
									<span>项目名称：</span>
									<form:input path="xmjbxx.xmmc" htmlEscape="false"
								...
								<div class="form-group right10">
									...
									<sys:treeselect id="sshy" name="xmjbxx.sshy"
										value="${deptRelationSearchParameter.xmjbxx.sshy}"
										labelName="sshyStr" notAllowSelectParent="true"
										labelValue="${fns:getTreeDictValue(deptRelationSearchParameter.xmjbxx.sshy)}"
								...
								<div
									...
									<span class="left10">总投资（万元）：</span> <input id="ztzMin"
										...
										value="${deptRelationSearchParameter.ztzMin}"
										style="width:120px;" /> - <input id="ztzMax" name="ztzMax"
										onchange="compareExchange('ztzMin','ztzMax')" type="number"
										class="form-control number"
										value="${deptRelationSearchParameter.ztzMax}"
										style="width:120px;" />
								</div>
								<div
									class="form-group ibox-custom-content ibox-custom-content-hide right10">
									<span>符合政府投资方向：</span>
									<sys:treeselect id="fhzftzfx" name="xmjbxx.fhzftzfx"
										value="${deptRelationSearchParameter.xmjbxx.fhzftzfx}"
										labelName="fhzftzfxStr"
										labelValue="${fns:getTreeDictValue(deptRelationSearchParameter.xmjbxx.fhzftzfx)}"
			...
			<form:select path="xmjbxx.szt" class="form-control" cssStyle="width:100px;">
				...
				<form:options items="${fns:getDictList('szt')}" itemLabel="label" itemValue="value" htmlEscape="false" />
								...
								<div
									class="form-group ibox-custom-content ibox-custom-content-hide right10">
									<span>审核辅助标志：</span>
									<form:input path="xmjbxx.shfzbsone" htmlEscape="false"
										maxlength="200" class="form-control input-sm" />
								</div>
								<%-- 该计划是否被挑选为投资计划项目 --%>
								<div
								...
								<div
									class="form-group ibox-custom-content ibox-custom-content-hide right10">
									<span>项目阶段：</span>
									<form:select path="xmjbxxSearchParameter.xmscjd"
										class="form-control">
										<form:option value="" label="" />
								...
								<div
									...
									<sys:treeselect id="jsdd" name="xmjbxx.jsdd"
										value="${deptRelationSearchParameter.xmjbxx.jsdd}"
										labelName="jsddStr"
								...
								<div
									class="form-group ibox-custom-content ibox-custom-content-hide right10">
									<span>来源单位：</span>
									<form:input path="xmjbxx.xmfrdw" htmlEscape="false"
										maxlength="200" class="form-control input-sm" />
								</div>
								<%-- <div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			<span>是否专项债：</span>
		...
		<div class="form-group ibox-custom-content ibox-custom-content-hide right10">
			...
			<form:select path="xmjbxx.sfppp" class="form-control">
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="pull-left">
							<shiro:hasPermission name="xmjbxx:xmjbxx:cbxmCbkList_qxcb">
								<table:operateRow url="${ctx}/xmjbxx/xmjbxx/cbxmCbkQxcb"
									id="contentTable" operateIndex="6" label="取消储备"
									checkFunc="checkQxcb"></table:operateRow>
								<!-- 取消储备 -->
							</shiro:hasPermission>
							<%-- <shiro:hasPermission name="xmjbxx:xmjbxx:cbxmCbkList_qxcb">
				<table:operateRow url="${ctx}/xmjbxx/xmjbxx/cbxmCbkThShq" id="contentTable" operateIndex="7" label="撤回"></table:operateRow><!-- 取消储备 -->
			</shiro:hasPermission> --%>
							<%--<shiro:hasPermission name="xmjbxx:xmjbxx:cbxmCbkList_tbcbxm">
			<table:operateRow url="${ctx}/xmjbxx/xmjbxx/cbxmCbkTbcbxm" id="contentTable" operateIndex="6" label="使用滚动计划同步储备项目"></table:operateRow>
			</shiro:hasPermission>--%>
							<table:viewRowWithCallback
								url="${ctx}/log/cInsBusinessLog/xmjbxxBusinessLogList"
								title="项目履历" id="contentTable" width="1000px" label="查看项目编报履历"></table:viewRowWithCallback>
							<%--<shiro:hasPermission name="xmjbxx:xmjbxx:cbxmCbkList_jiasuo">
				<table:operateRow url="${ctx}/xmjbxx/xmjbxx/cbxmCbk_jiasuo" id="contentTable" operateIndex="4" label="加锁" checkFunc="checkJiasuo"></table:operateRow><!-- 加锁按钮 -->
			</shiro:hasPermission>
			<shiro:hasPermission name="xmjbxx:xmjbxx:cbxmCbkList_jiesuo">
				<table:operateRow url="${ctx}/xmjbxx/xmjbxx/cbxmCbk_jiesuo" id="contentTable" operateIndex="5" label="解锁" checkFunc="checkJiesuo"></table:operateRow><!-- 解锁按钮 -->
			</shiro:hasPermission>--%>
							...
							<c:forEach items="${page.list}" var="deptRelation">
								...
								<tr>
									<td><input type="checkbox" id="${xmjbxx.id}"
										xmmcAttr="${xmjbxx.xmmc}" sztAttr="${xmjbxx.szt}"
										nrztAttr="${deptRelation.nrzt}"
										userGradeAttr="${fns:getUserOffice().grade}" class="i-checks">
									</td>
									<td><a href="#"
